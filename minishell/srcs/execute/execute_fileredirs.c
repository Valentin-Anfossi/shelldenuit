...
/*   Updated: 2025/07/05 14:40:00 by vanfossi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ms_fileredir_outfd(t_redir *r, int *out_fd)
{
	if (r->type == R_OUT)
	{
		if (*out_fd != -1)
			close (*out_fd);
		*out_fd = open(r->target, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC,
				0777);
		if (*out_fd < 0)
			return (1);
	}
	else if (r->type == R_APPEND)
	{
		if (*out_fd != -1)
			close(*out_fd);
		*out_fd = open(r->target, O_WRONLY | O_APPEND | O_CREAT | O_CLOEXEC,
				0777);
		if (*out_fd < 0)
			return (1);
	}
